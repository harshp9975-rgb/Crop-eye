...
--------------------------
Searches News (Google News RSS), Reddit, and YouTube for new mentions of
medicinal/aromatic/nutraceutical crops, deduplicates against previously seen
items, emails a digest of anything new, and regenerates a static dashboard.

Designed to run on a schedule via GitHub Actions (see .github/workflows/monitor.yml).
All secrets (email creds, YouTube API key) are read from environment variables.
"""
...
DASHBOARD_FILE = "docs/index.html"  # docs/ so GitHub Pages can serve it directly
MAX_SEEN_ITEMS = 5000  # cap the dedupe store so it doesn't grow forever
REQUEST_TIMEOUT = 15
...
def save_seen(seen):
    # Trim oldest entries if it grows too large
...
def send_email(new_items):
    if not (GMAIL_ADDRESS and GMAIL_APP_PASSWORD and ALERT_EMAIL_TO):
        print("Email not configured (missing GMAIL_ADDRESS / GMAIL_APP_PASSWORD) — skipping send.")
        return
    if not new_items:
